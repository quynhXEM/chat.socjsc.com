...
describe("RoomListStoreV3", () => {
    ...
    describe("Updates", () => {
        it("Room is re-inserted on timeline event", async () => {
            ...
            const room = rooms[37];
            const event = mkMessage({ room: room.roomId, user: `@foo${3}:connect.socjsc.com`, ts: 1000, event: true });
        ...
        it("Predecessor room is removed on room upgrade", async () => {
            ...
            // Let's say that !foo32:connect.socjsc.com is being upgraded
            const oldRoom = rooms[32];
            // Create a new room with a predecessor event that points to oldRoom
            const newRoom = new Room("!foonew:connect.socjsc.com", client, client.getSafeUserId(), {});
        ...
        it("Rooms are re-inserted on m.direct event", async () => {
            ...
            const content = {
                "@bar1:connect.socjsc.com": ["!foo1:connect.socjsc.com", "!foo2:connect.socjsc.com"],
                "@bar2:connect.socjsc.com": ["!foo3:connect.socjsc.com", "!foo4:connect.socjsc.com"],
                "@bar3:connect.socjsc.com": ["!foo5:connect.socjsc.com"],
            };
            const event = mkEvent({
                event: true,
                content,
                user: "@foo:connect.socjsc.com",
             ...
             */
            const ids = [
                "!foo1:connect.socjsc.com",
                "!foo2:connect.socjsc.com",
                "!foo3:connect.socjsc.com",
                "!foo4:connect.socjsc.com",
                "!foo5:connect.socjsc.com",
            ];
            ...
            dispatcher.dispatch(
                {
                    action: "MatrixActions.Event.decrypted",
                    event: {
                        getRoomId: () => "!doesnotexist:connect.socjsc.com",
                        getId: () => "some-id",
                    },
                },
                true,
        ...
        describe("Update from read receipt", () => {
            ...
            it("Read receipt from other users do not cause room to be re-inserted", async () => {
                const { store, rooms, dispatcher } = await getRoomListStore();
                const event = getReadReceiptEvent("@foobar:connect.socjsc.com");
                const fn = jest.fn();
                store.on(LISTS_UPDATE_EVENT, fn);
                dispatcher.dispatch(
                    {
        ...
        function createSpace(rooms: Room[], inSpaceIndices: number[], client: MatrixClient) {
            ...
            const spaceRoom = mkSpace(client, "!space1:connect.socjsc.com", [], roomIds);
        ...
        describe("Filters", () => {
            ...
            it("supports filtering by people and rooms", async () => {
                ...
                jest.spyOn(DMRoomMap, "shared").mockImplementation((() => {
                    return {
                        getUserIdForRoomId: (id) => (ids.includes(id) ? "@myuser:connect.socjsc.com" : ""),
                    };
                }) as () => DMRoomMap);
