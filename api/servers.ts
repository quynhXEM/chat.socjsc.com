...
module.exports = async (req: VercelRequest, res: VercelResponse): Promise<void> => {
  ...
  try {
    ...
    const url = `https://soc.socjsc.com/items/connect_server?filter[app_id]=${encodeURIComponent(APP_ID)}&filter[status]=published&limit=${encodeURIComponent(limit)}&fields=${encodeURIComponent(fields)}&meta=${encodeURIComponent(meta)}`;
