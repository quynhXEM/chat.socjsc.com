...
test.describe("Share dialog", () => {
    ...
    test("should share a room", { tag: "@screenshot" }, async ({ page, app, room }) => {
        ...
        await expect(dialog.getByText(`https://chat.socjsc.com/#/${room.roomId}`)).toBeVisible();
    ...
    test("should share a room member", { tag: "@screenshot" }, async ({ page, app, room, user }) => {
        ...
        await expect(dialog.getByText(`https://chat.socjsc.com/#/${user.userId}`)).toBeVisible();
